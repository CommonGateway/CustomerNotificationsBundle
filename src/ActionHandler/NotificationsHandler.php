    ...
    public function getConfiguration(): array
    {
        return [
            ...
            'properties'  => [
                ...
                'emailConfig'        => [
                    'type'        => 'array',
                    'description' => 'Configuration for sending an email after the notification has been received. If not present it will not be possible for emails to be send. "getObjectDataConfig" can be used to configure how to find and add the data of one ObjectEntity to the email message. "throw" is the event we should throw to trigger another EmailHandler action that will send the actual email. Basic details about how "getObjectDataConfig" works can be found in the description & example of the extraConditions property for this action->configuration. Good to know: "source" = reference of the source to call. "sourceProperties" = this is the array with property names to get from the response of the source. "searchSchemas" = array with Schema references to use when searching an Object in de Gateway. "searchQuery" = query array to use when searching an Object in de Gateway, use {{sourcePropertyName}} here to insert the values got using "sourceProperties. Note that it also possible to use "getObjectDataConfig" recursively, see example of extraConditions for how this is done. Make sure to add the "forParentProperties" array and add the property to it that has an url in the value and can be used to call another (or the same) source again..',
                    'example'     => '"emailConfig":{"getObjectDataConfig":{"source":"https://buren.nl/source/buren.zrc.source.json","sourceEndpoint":"/rollen","sourceQuery":{"zaak":"{{body.hoofdObject}}","omschrijvingGeneriek":"initiator"},"sourceProperties":["results.0.betrokkeneIdentificatie.inpBsn"],"searchSchemas":["https://commongateway.nl/klant.partij.schema.json"],"searchQuery":{"externeIdentificaties.partijIdentificator.objectId":"{{results.0.betrokkeneIdentificatie.inpBsn}}","externeIdentificaties.partijIdentificator.objecttype":"ingeschrevenpersoon"}},"throw":"notifications.zaak.created.email"}',
                    'nullable'    => true,
                ],
                'smsConfig'          => [
                    'type'        => 'array',
                    'description' => 'Configuration for sending an sms after the notification has been received. If not present it will not be possible for sms to be send. "getObjectDataConfig" can be used to configure how to find and add the data of one ObjectEntity to the sms message, if set to "sameAsEmail" the same object (response from sources) as for email will be used (or the same configuration). "throw" is the event we should throw to trigger another SMSHandler action that will send the actual sms. For more details about how "getObjectDataConfig" works, please see the emailConfig property of this action->configuration.',
